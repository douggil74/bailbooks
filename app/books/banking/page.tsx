  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-500 mb-1">Total Balance (All Accounts)</p>
        <p className="text-3xl font-bold text-white">{fmt(totalBalance)}</p>
        <p className="text-xs text-gray-500 mt-1">{accounts.length} active account{accounts.length !== 1 ? 's' : ''}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-12 text-center">
          ...
          <p className="text-gray-600 text-sm mt-1">Add your first bank account to start tracking balances</p>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 gap-4">
          {accounts.map((acct) => {
            ...
            return (
              <div key={acct.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5 hover:border-gray-600 transition-colors group">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-lg bg-gray-800 flex items-center justify-center ${meta.color}`}>
                      <Icon className="w-5 h-5" />
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="text-white font-semibold text-sm">{acct.account_name}</p>
                        {acct.is_default && <Star className="w-3.5 h-3.5 text-[#d4af37] fill-[#d4af37]" />}
                      </div>
                      <p className="text-gray-500 text-xs">{acct.bank_name} &middot; {meta.label}</p>
                    </div>
                  </div>
                ...
                <div className="flex items-end justify-between">
                  <div>
                    <p className="text-2xl font-bold text-white">{fmt(Number(acct.current_balance))}</p>
                  </div>
                  {acct.account_number_last4 && (
                    <p className="text-xs text-gray-600">&bull;&bull;&bull;&bull; {acct.account_number_last4}</p>
                  )}
                </div>
              </div>
            );
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <form onSubmit={handleSubmit} className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-md space-y-4">
        <h2 className="text-lg font-bold text-white">{account ? 'Edit Account' : 'Add Bank Account'}</h2>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="text-xs text-gray-400 block mb-1">Last 4 of Account #</label>
            <input
